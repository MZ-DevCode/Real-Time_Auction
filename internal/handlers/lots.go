...
func GetLotsHandlers(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		query := "SELECT id, title, description, start_price, current_price, ends_at, status FROM lots"		rows, err := database.DB.QueryContext(ctx, query)
		if err != nil{
	...
	for rows.Next() {
		...
		err := rows.Scan(&id, &title, &description, &price, &time, &status)
		if err != nil {
